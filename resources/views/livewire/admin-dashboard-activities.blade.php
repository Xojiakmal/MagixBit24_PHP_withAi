<div wire:poll.5s class="mt-8">
                ...
                <div class="flex space-x-3 items-start border-l-2 border-accent/30 pl-4 py-1 relative">
                    ...
                    <div>
                        <p class="text-sm text-gray-300">
                            <strong class="text-white">{{ $activity->user->name ?? 'Tizim' }}</strong> 
                            @if($activity->action == 'created') yaratdi
                            @elseif($activity->action == 'deleted') o'chirdi
                            @elseif($activity->action == 'assigned') biriktirdi
                            @elseif($activity->action == 'status_changed') holatini o'zgartirdi
                            @else amalini bajardi
                            @endif
            ...
            @forelse($history as $hist)
                <div class="bg-white/5 p-3 rounded-xl border border-white/5">
                    <div class="flex justify-between mb-2">
                        <p class="text-sm font-semibold text-white">{{ $hist->user->name ?? 'Tizim' }}</p>
                        <span class="text-xs text-dark-muted">{{ $hist->created_at->diffForHumans() }}</span>
                    </div>
                    <p class="text-xs text-gray-400 mb-2">
                        Obyekt: <span class="text-blue-400">{{ class_basename($hist->subject_type) }} #{{ $hist->subject_id }}</span>
                    </p>
                    <div class="bg-black/30 rounded-lg p-2 text-xs font-mono break-words space-y-1">
                        ...
                        @endforeach
                        @if($changesCount > 2)
                            <div class="text-dark-muted text-[10px] mt-1 italic">+ yana {{ $changesCount - 2 }} ta o'zgarish</div>
    ...
    <x-slide-over wire:model="isExpandedActivity" id="expandedActivity" title="Barcha Faoliyatlar" maxWidth="4xl">
        <x-slot:actions>
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between">
                                <p class="text-base text-gray-200">
                                    <strong class="text-white">{{ $activity->user->name ?? 'Tizim' }}</strong> 
                                    @if($activity->action == 'created') yangi ma'lumot yaratdi
                                    @elseif($activity->action == 'deleted') ma'lumotni o'chirdi
                                    @elseif($activity->action == 'assigned') xodim biriktirdi
                                    @elseif($activity->action == 'status_changed') holatini o'zgartirdi
                                    @else amalini bajardi
                                    @endif
                                </p>
                            ...
                            <p class="text-sm text-accent mt-1">Obyekt turi: {{ class_basename($activity->subject_type) }} | Obyekt ID: {{ $activity->subject_id }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <x-slide-over wire:model="isExpandedHistory" id="expandedHistory" title="Barcha O'zgarishlar Tarixi" maxWidth="5xl">
        <x-slot:actions>
                    ...
                    <tbody class="divide-y divide-dark-border/50">
                        @foreach($history as $hist)
                            <tr class="hover:bg-white/5 transition-colors">
                                ...
                                <td class="px-6 py-4 font-medium text-white">{{ $hist->user->name ?? 'Tizim' }}</td>
